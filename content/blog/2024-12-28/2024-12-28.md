...
{% image "pds-this-is-an-at-protocol.png", "A screenshot of the Bluesky PDS page", [500] %}
...
2.  Go to Networks → Tunnels <br/>
    {% image "networks-tunnels.png", "A screenshot of Cloudflare Dashboard showing Tunnels menu item in Networks group", [300] %}

3.  Click "Create a tunnel" <br/>
    {% image "your-tunnels-create-tunnel.png", "A screenshot of Cloudflare website showing the '+ Create a tunnel' button", [300] %}

4.  Select Cloudflared <br/>
    {% image "cloudflared.png", "A screenshot of Cloudflare Tunnel showing the 'Select Cloudflared' button", [300] %}

5.  Name your tunnel <br/>
    {% image "name-your-tunnel.png", "A screenshot of Cloudflare Tunnel page, showing the textbox labeled 'Name your tunnel'", [300] %}

6.  Choose Docker as your environment, and copy the command to run a connector <br/>
    {% image "choose-your-environment.png", "A screenshot of Cloudflare Tunnel to select an environment with Docker highlighted", [300] %}
    {% image "install-and-run-connector.png", "A screenshot of Cloudflare Tunnel showing a docker command to run cloudflared", [300] %}
    </div>
    You can start the connector (`cloudflared`) by running the command. I usually modify the command and add a daemon flag (`-d`) so that it runs in the background, that is:
    `docker run -d cloudflare/cloudflared:latest ...`
    Click "Next"

7.  Configure the public hostname and service

    {% image "add-public-hostname.png", "A screenshot of Cloudflare Tunnel showing a screen to add a public hostname", [300] %}
...
1. Open a Blusky client (I used my iOS app)
   ...
   {% image "bluesky-start-screen.png", "A screenshot of Bluesky iOS app startup screen with Create account & Sign in buttons", [300] %}

   Select Create account

2. Change hosting provider to the domain that you set </br>
   {% image "bs-your-account.png", "A screenshot of Bluesky create-account screen with hosting provider set to bluesky.namisunami.com", [300] %}

3. Copy and paste the invite code </br>
   {% image "invite-code.png", "A screenshot of Bluesky create-account screen with an invite code", [300] %}
...
5. Set the username. By default, the handle will look like `@your_username.yourdomain.com`. In my case, I wanted my handle to be my top domain name, that is `@namisunami.com`. I will describe the process in the next step. For now, you can put a temporary username here, and you can change it later. Click "Next" </br>
   {% image "choose-your-username.png", "A screenshot of Bluesky for choosing your username", [300] %}

#### Changing the handle

By default, your handle will show as invalid. </br>
{% image "bs-nami-invalid-handle.jpg", "A screenshot of Nami's Bluesky account showing an Invalid Handle warning", [300] %}
...
1. Go to Settings → Account → Handle </br>
   {% image "change-handle.png", "A screenshot of Bluesky's change handle view", [300] %}

2. Select "I have my own domain →" </br>
   {% image "change-handle-own-domain.png", "A screenshot of Bluesky's change handle view to specify own domain's handle", [300] %}
...
3. Fill in the handle that you want to use. Then you have the information to set up the TXT record for your domain. You can do so via Cloudflare's DNS settings ([Cloudflare docs](https://developers.cloudflare.com/dns/manage-dns-records/how-to/create-dns-records/)). </br>
   {% image "cloudflare-dns-settings.png", "A screenshot of Cloudflare's DNS setting", [300] %}
...
4. Once you set up your DNS record on Cloudflare, click "Verify DNS Record". Then click "Update to YOUR_HANDLE" to complete the setup. </br>
   {% image "domain-verified.png", "A screenshot of Bluesky client showing a green checkmark with 'Domain verified' text, with a button labeled, 'updated to namisunami.com'", [300] %}
